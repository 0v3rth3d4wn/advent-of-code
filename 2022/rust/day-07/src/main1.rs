...
fn get_current_directory<'a>(
    file_system: &'a mut FileSystem,
    pwd: &Vec<String>,
) -> &'a mut Directory {
    let mut curdir = file_system
        .directories
        .get_mut(pwd.first().unwrap())
        .unwrap();
...
fn parse_terminal_line(line: &str) -> TerminalLine {
    match &line[0..4] {
        ...
        "dir " => TerminalLine::Directory(Directory {
...
fn generate_filesystem() -> FileSystem {
    let lines: Vec<&str> = include_str!("../data.txt").lines().collect();
    ...
    for line in lines.iter().skip(2) {
        match parse_terminal_line(line) {
            ...
            TerminalLine::File(file) => {
                get_current_directory(&mut file_system, &pwd)
                    .files
